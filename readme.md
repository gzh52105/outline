...
* 在微信小程序中使用云开发
    ...
    * 使用步骤：
        1. 初始化
            > 在调用云开发各 API 前，需先调用初始化方法 init（只需要执行一次），不管是在小程序端还是云函数端，都需要初始化
            ```js
                //  wx.cloud
                wx.cloud.init({
        ...
        2. 操作云开发资源
            ...
            * 云函数
                > 云函数是一段运行在云端（服务端）的代码，运行在NodeJS环境下，可直接使用npm模块，编写好的云函数可在小程序端调用
                * 创建云函数
                    > 运行在NodeJS环境下，遵循commonJS模块化规范
                * 调试云函数
                    * 本地调试（重点）
                    * 云端调试
                * 部署云函数：上线
...
### 知识点
* 跨平台框架
    > 原理：利用框架自带的编译器，把代码编译到各个平台，实现一次编写到处部署
    * wePY
...
* 应用类型
    ...
    * 小程序
        > 使用web技术
* Hybrid开发模式
    > 综合NativeApp与WebApp的优点
    * Native工程师主导(主流)
        > web工程师开发页面与**业务逻辑**，然后交给Native工程师打包
        * navtive工程师：
            * 提供硬件接口
            * 打包
            * 发布
